...
class V3Layer : public V11Geometry
{
  ...
  private:
    ...
    TGeoVolume *createStave(Double_t phi,const TGeoManager *mgr = gGeoManager);
    ...
    /// \param mgr The GeoManager (used only to get the proper material)
    TGeoVolume *createStaveOuterB(Double_t phi,const TGeoManager *mgr = gGeoManager);// Cristina

    /// Create dummy stave
    /// \param mgr The GeoManager (used only to get the proper material)
    TGeoVolume *createStaveModelOuterBDummy(Double_t phi,const TGeoManager *mgr = gGeoManager) const;  //Cristina

    /// Create the mechanical half stave structure or the Outer Barrel as in TDR
    /// \param mgr The GeoManager (used only to get the proper material)
    TGeoVolume *createStaveModelOuterB2(Double_t phi,const TGeoManager *mgr = gGeoManager); //Cristina
